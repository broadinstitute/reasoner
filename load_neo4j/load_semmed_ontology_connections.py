...
def add_cui_connection(tx, origin_cui, origin_type, target_cui,
                       target_type, target_name, predicate, pred_count):
    tx.run("MERGE (o:%s {cui: {origin_cui}}) "
           "MERGE (t {cui: {target_cui}}) "
           "SET t.name = {target_name} "
           "SET t:%s "
           "MERGE (o)-[r:%s {source: 'semmeddb_iter2'}]->(t) "
           "SET r.count = {pred_count}" % (origin_type,
                                           target_type,
                                           predicate),
           origin_cui=origin_cui, target_cui=target_cui,
           target_name=target_name, pred_count=pred_count)
...
def load_connections(session, subject_cui, subject_type, connections):
    for connection in connections:
        ...
        # handle ill-formatted cuis from SemMedDB
        if '|' in connection['object_cui']:
            object_cui = connection['object_cui'].split('|')[0]
            object_name = connection['object_name'].split('|')[0]
        else:
            object_cui = connection['object_cui']
            object_name = connection['object_name']

        print(subject_cui, subject_type, object_cui,
              connection['predicate'], connection['count'])
        session.write_transaction(add_cui_connection,
                                  subject_cui,
                                  subject_type,
                                  object_cui,
                                  connection['object_semtype'],
                                  object_name,
...
db = mysql.connector.connect(user=config['semmeddb']['user'],
